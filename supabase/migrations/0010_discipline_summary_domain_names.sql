-- Extends discipline_summary() (0006) to also return the discipline's
-- domain names -- names only, never weight_pct -- so the Pre-Assessment
-- Instructions screen (UX_Wireframes_User_Flow.md section 3.4) can list
-- "Covers: ..." without querying discipline_skill_weights directly, which
-- candidates have no RLS access to (by design, 0005_rls_policies.sql).
--
-- Postgres won't let CREATE OR REPLACE change a RETURNS TABLE column set,
-- so this drops and recreates the function; grants must be reapplied.
drop function if exists discipline_summary(uuid);

create function discipline_summary(p_discipline_id uuid)
returns table (total_questions int, estimated_minutes int, domain_names text[])
language plpgsql
security definer
...
begin
  ...
  return query
    select v_total_questions, (ceil((v_total_seconds / 60.0) / 5.0) * 5)::int, v_domain_names;
end;
$$;
